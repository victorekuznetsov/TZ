...
const AnalyticsCore = (() => {
  ...
  function planningControl(rows, asOf) {
    ...
    return { cur, next, chain, curYear, notReleasedStarted, unplanned, nextPlan, approvedPlan, approvedShare: ratio(approvedPlan, nextPlan),
  ...
  function planScope(rows, asOf) {
    ...
    rows.forEach(r => {
      ...
      let t = by.get(g);
      if (!t) by.set(g, (t = { group: g, name: g ? (PG_NAMES[g] || g) : "не указана", dev: DEV_GROUPS.includes(g), codes: new Set(), years: {} }));
  ...
  function deck(src, ctx, m) {
    ...
    const items = src.provision.items || [], noCtx = !ctx || !(ctx.site || ctx.model || ctx.unit || ctx.order);
    ...
    ((src.usoWk && src.usoWk.orders) || []).forEach(o => {
      ...
      t.plan += N(o.planValue); t.fact += N(o.factValue); t.open += N(o.openValue); t.orders += 1; t.closed += o.closed ? 1 : 0;
    });
    return {
  ...
  return { YEARS, STAGES, GROUP, GROUPS, T, RULES, DEV_GROUPS, PG_NAMES, isDevGroup, planScope, decodeControl, inContext, byYear, execution, deck,
