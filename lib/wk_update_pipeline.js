...
const WkUpdate = (() => {
  ...
  /* Распределение с привязкой к площадкам — как allocate в build_provision.py:
     проход 0 — свой склад под работы раньше срока поставки кода;
     проход 1 — по дате: свой склад, склад другой площадки (перемещение,
     transfer), закупка к сроку; проход 2 — остатки приходов. */
  function allocate(need, stock, today, leadDefault) {
    const avail = {};
    const inflow = {};
    ...
    need.forEach(r => {
      ...
      r.transfer = 0; r.transferFrom = {}; r.left = r.qty;
    });
    const horizon = code => {
      const lead = ((stock[code] || {}).purchase || {}).leadDays || leadDefault || 0;
      if (!today) return "9999";
      const d = new Date(today); d.setUTCDate(d.getUTCDate() + lead);
      return iso(d);
    };
    need.forEach(r => {
      if (!r.date || r.date > horizon(r.code)) return;
      const pool = avail[r.code] || {}, own = needSite(r);
      const take = Math.min(r.left, pool[own] || 0);
      if (take > 0) { pool[own] -= take; r.fromStock += take; r.left -= take; }
    });
    need.forEach(r => {
      const pool = avail[r.code] || {}, own = needSite(r);
      ...
      Object.keys(pool).sort((a, b) => (pool[b] - pool[a]) || a.localeCompare(b)).forEach(st => {
        if (r.left <= 0 || st === own) return;
        const take = Math.min(r.left, pool[st]);
        if (take <= 0) return;
        pool[st] -= take; r.fromStock += take; r.transfer += take; r.left -= take;
  ...
  function buildOrders(rows, names) {
    ...
    grouped.forEach(lines => {
      ...
      const detail = lines.slice().sort((a, b) => (a.date || "9999").localeCompare(b.date || "9999") || a.work.localeCompare(b.work, "ru") || String(a.code).localeCompare(String(b.code))).map(r => {
        ...
        row.transfer = Math.round((r.transfer || 0) * 1000) / 1000;
        row.transferValue = Math.round(val(r, "transfer") * 100) / 100;
  ...
  function assemble(details, ekmtrWk, stockJson) {
    ...
    const need = allocate(loaded.need, stock, today, leadDefault);
    ...
    const wkClosed = loaded.closed.filter(r => r.code in wkNames).map(r => {
      r.fromStock = 0; r.fromBuy = 0; r.late = 0; r.undated = 0; r.gap = 0;
      r.transfer = 0; r.transferFrom = {}; r.left = 0;
      return r;
    });
    ...
    agg.forEach((a, code) => {
      ...
      items.push({
        ...
        transfer: Math.round(a.transfer * 1000) / 1000,
        transferValue: Math.round(a.transferValue * 100) / 100,
