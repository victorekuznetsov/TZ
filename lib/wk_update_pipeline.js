...
const WkUpdate = (() => {
  ...
  function loadNeed(details) {
    ...
    for (const d of details) {
      ...
      grains.forEach(g => {
        const remaining = Math.max(Math.max(g.planQty, 0) - Math.max(g.factQty, 0), 0);
        if (!g.code) {
          nocodeRows++;
  ...
  /* Площадка по коду завода — как plant_site в build_stock.py. */
  ...
  function plantSite(plant) {
    const p = String(plant == null ? "" : plant).trim().toUpperCase();
    return PLANT_SITE[p] || PREFIX_SITE[p.slice(0, 2)] || "";
  }
  const needSite = r => plantSite(r.site) || String(r.site || "");

  /* Распределение с привязкой к площадкам — как allocate в build_provision.py:
     склад покрывает только свою площадку; проход 1 — по дате: свой склад,
     закупка к сроку; проход 2 — остатки приходов. Перемещение между
     площадками ограничено и в покрытие не входит: отдельно считается
     возможность (transferPotential) — запас других площадок, оставшийся
     после их потребности, против непокрытого к сроку. */
  function allocate(need, stock, today) {
    ...
    Object.keys(stock).forEach(c => {
      const s = stock[c];
      avail[c] = s.bySite && Object.keys(s.bySite).length
        ? Object.fromEntries(Object.entries(s.bySite).map(([st, o]) => [st, N(o.availQty)]))
        : { "": N(s.availQty) };
    ...
    need.forEach(r => {
      r.fromStock = 0; r.fromBuy = 0; r.late = 0; r.undated = 0; r.gap = 0;
      r.transferPotential = 0; r.transferFrom = {}; r.left = r.qty;
    });
    need.forEach(r => {
      const pool = avail[r.code] || {}, own = needSite(r);
      const takeOwn = Math.min(r.left, pool[own] || 0);
      if (takeOwn > 0) { pool[own] -= takeOwn; r.fromStock += takeOwn; r.left -= takeOwn; }
      const month = (r.date || "").slice(0, 7);
      const sched = r.ppm === "never" ? null : inflow[r.code];
    ...
    need.forEach(r => {
      let open = r.left + r.late + r.undated;
      if (open <= 0) return;
      const pool = avail[r.code] || {}, own = needSite(r);
      Object.keys(pool).sort((a, b) => (pool[b] - pool[a]) || a.localeCompare(b)).forEach(st => {
        if (open <= 0 || st === own) return;
        const take = Math.min(open, pool[st]);
        if (take <= 0) return;
        pool[st] -= take; open -= take; r.transferPotential += take;
        r.transferFrom[st] = (r.transferFrom[st] || 0) + take;
      });
    });
  ...
  function totals(rows) {
    ...
    TKEYS.forEach(k => { t[k] = 0; t[k + "Qty"] = 0; });
    rows.forEach(r => {
      t.value += r.value; t.qty += r.qty;
      TKEYS.forEach(k => { t[k] += val(r, k); t[k + "Qty"] += r[k] || 0; });
    });
  ...
  function buildOrders(rows, names) {
    ...
    grouped.forEach(lines => {
      ...
      const detail = lines.slice().sort((a, b) => (a.date || "9999").localeCompare(b.date || "9999") || a.work.localeCompare(b.work, "ru") || String(a.code).localeCompare(String(b.code))).map(r => {
        ...
        row.transferPotential = Math.round((r.transferPotential || 0) * 1000) / 1000;
        row.transferPotentialValue = Math.round(val(r, "transferPotential") * 100) / 100;
        row.transferFrom = Object.fromEntries(Object.entries(r.transferFrom || {}).map(([st, q]) => [st, Math.round(q * 1000) / 1000]));
        return row;
      });
  ...
  function assemble(details, ekmtrWk, stockJson) {
    ...
    const wkClosed = loaded.closed.filter(r => r.code in wkNames).map(r => {
      r.fromStock = 0; r.fromBuy = 0; r.late = 0; r.undated = 0; r.gap = 0;
      r.transferPotential = 0; r.transferFrom = {}; r.left = 0;
      return r;
    });
    ...
    known.forEach(r => {
      ...
      if (!a) { a = { needQty: 0, needValue: 0, transferFrom: {} }; TKEYS.forEach(k => { a[k] = 0; a[k + "Value"] = 0; }); agg.set(r.code, a); }
      a.needQty += r.qty; a.needValue += r.value;
      TKEYS.forEach(k => { a[k] += r[k]; a[k + "Value"] += val(r, k); });
      Object.entries(r.transferFrom).forEach(([st, q]) => { a.transferFrom[st] = (a.transferFrom[st] || 0) + q; });
    ...
    agg.forEach((a, code) => {
      ...
      items.push({
        ...
        availBySite: Object.fromEntries(Object.entries(s.bySite || {}).map(([st, o]) => [st, o.availQty || 0])),
        transferPotential: Math.round(a.transferPotential * 1000) / 1000,
        transferPotentialValue: Math.round(a.transferPotentialValue * 100) / 100,
        transferFrom: Object.fromEntries(Object.entries(a.transferFrom).map(([st, q]) => [st, Math.round(q * 1000) / 1000])),
        openQty: p.openQty || 0,
    ...
    const orders = allOrders.filter(o => o.qty > 1e-9);
    const closedOrders = allOrders.filter(o => o.qty <= 1e-9);
  ...
  function buildUsoWk(rows, props, labels) {
    ...
    (rows || []).forEach(r => {
      ...
      byOrder.get(key).lines.push({
        ...
        qp: N(r.qp), p: N(r.p), price: N(r.qp) ? Math.round(N(r.p) / N(r.qp) * 100) / 100 : 0,
      });
    });
    // реестр — состав МТР подрядчика (кол-во, цена, сумма); «факт (подр.)» реестра — плановая цена в валюте
    // по курсу, не факт: статус и факт берутся из заказа ТОРО (AnalyticsCore.usoStatus)
  ...
  function mergeUsoWk(current, fresh, years) {
    ...
    ["closedOrders", "openValue", "factValue"].forEach(k => delete merged.meta[k]);
    return merged;
  }
