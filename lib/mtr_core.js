...
const MtrCore = (() => {
  ...
  /* группа планирования ТОРО заказа: «Развитие» планирует только 100 Механика и 200 Энергетика */
  const DEV_GROUPS = ["100", "200"];
  function devMap(controlRows) {
    const m = new Map();
    (controlRows || []).forEach(r => m.set(r.y + "|" + r.order, DEV_GROUPS.includes(String(r.pg || "").split("/").pop())));
    return m;
  }
  ...
  const pgMatch = (pg, f) => (!f ? true : f === "dev" ? DEV_GROUPS.includes(pgGroup(pg))
    : f === "be" ? !DEV_GROUPS.includes(pgGroup(pg)) : pgGroup(pg) === String(f));
  function inCtx(row, ctx, pgOf) {
    if (!ctx) return true;
    if (ctx.pg && !pgMatch((pgOf && pgOf.get(String(row.order))) || "", ctx.pg)) return false;
  ...
  function efficiency(src, ctx) {
    ...
    const us = unitSites(src.fleet), stg = stageMap(src.controlRows), dev = devMap(src.controlRows);
    const pgOf = new Map((src.controlRows || []).filter(r => r.pg).map(r => [String(r.order), r.pg]));
    const rows = (src.scheduleRows || []).map(r => ({ r, site: siteOfRow(us, r) }))
      .filter(x => inCtx({ site: x.site, model: x.r.model, unit: x.r.unit, order: x.r.order }, ctx, pgOf));
    const years = [...new Set((src.scheduleRows || []).map(r => r.year))].sort();
    const win = years.filter(y => y >= String(+cur - 2) && y <= cur);            // окно расхода: 3 последних года
    const factYears = years.filter(y => y <= cur);
    const noCtx = !(ctx.site || ctx.model || ctx.unit || ctx.order || ctx.pg);
    ...
    (src.provision.orders || []).forEach(o => {
      ...
      if (!inCtx({ site, model: o.model, unit: o.unit, order: o.order }, ctx, pgOf)) return;
      (o.lines || []).forEach(l => {
    ...
    // 4. внеплановый расход и неиспользованный план по строкам графика — оценка планирования
    //    «Развития»: только заказы групп планирования 100/200 (группа известна с 2024 года)
    const unplanned = {};
    rows.forEach(({ r }) => {
      if (!dev.get(r.year + "|" + r.order)) return;
      const t = unplanned[r.year] || (unplanned[r.year] = { fact: 0, unplanned: 0, plan: 0, unused: 0, lines: 0, unplannedLines: 0 });
  ...
  return { DEV_GROUPS, PRICE_JUMP, normArt, interKey, resolve, profile, efficiency, statusOf };
})();
