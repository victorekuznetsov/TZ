...
const StockPipeline = (() => {
  ...
  function assemble(ekmtrWk, stock, restr, purch, srcNames) {
    ...
    // порядок позиций — как sorted(codes) в build_stock.py
    const codes = [...new Set([...Object.keys(stock.q), ...Object.keys(restr.q), ...Object.keys(purch.byCode)])].sort((x, y) => (x < y ? -1 : x > y ? 1 : 0));
    const items = [];
    ...
    return {
      meta: {
        ...
        purchaseDocumentSource: "TOPO/rawdata/Запас-Закупка/" + srcNames.purchase,
        currency: "RUB",
