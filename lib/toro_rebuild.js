...
const ToroRebuild = (() => {
  ...
  /* nodes: заказ → [машина, ТМ, узел, № ЕО узла] — заказы на ЕО-узлах (ковш, ЭД…) техместа машины WK
     (data/wk_nodes.json, build_wk_nodes.py); машина строки — ЕО с «WK», иначе — по карте */
  function scheduleRowsFromDetail(d, nodes) {
    const rows = new Map();
    let wk = 0;
    const s = String(d.s), y = String(d.y), file = `${s}_${y}.json`;
    const NODES = nodes || {};
    for (let i = 0; i < d.n; i++) {
      let unit = dictVal(d, "e", "ei", i), node = "";
      const order = String(d.o[i]);
      if (!WK_RE.test(unit)) {
        if (!NODES[order]) continue;
      ...
      if (!r) {
        ...
        r = {
          ...
          site: s, year: y, unit, model: "WK-" + (m ? m[1].toUpperCase() : "?"), order, work, code, name, node, start, end,
  ...
  function createMetaCollector(site0) {
    ...
    function header(cells) {
      ...
      C = {
        ...
        tm: col("Техническое место"), tmn: col("Техническое место", 1),
      };
    }
    ...
    function pushRow(cells) {
      ...
      if (!g) {
        ...
        g = {
          ...
          eo: str(get(cells, "eo")).trim(), tm: str(get(cells, "tmn")).trim(),
        };
        orders.set(o, g);
      }
      if (!g.text) {
    ...
    function finish() {
      ...
      // узлы машин WK: техместо, на котором стоит ЕО экскаватора, → машина; заказы на другие ЕО этого ТМ
      // (или вложенного) — ковш, ЭД, рукоять… — относятся к машине (как build_wk_nodes.py по БДО)
      const real = c => !!c && c !== "#";
      const machineTm = new Map();
      orders.forEach(v => { if (real(v.tm) && WK_RE.test(v.eo) && !machineTm.has(v.tm)) machineTm.set(v.tm, v.eo); });
      const rootOf = c => { const p = c.split("-"); for (let i = p.length; i > 0; i--) { const k = p.slice(0, i).join("-"); if (machineTm.has(k)) return k; } return null; };
      const nodes = {};
      orders.forEach((v, o) => {
        if (!real(v.tm) || WK_RE.test(v.eo)) return;
        const r = rootOf(v.tm);
        if (r) nodes[o] = [machineTm.get(r), v.tm, v.eo, v.eon];
      });
      return { orders, comps, originals, nodes };
    }
  ...
  function rebuild({ base, uploads = [], stockJson, ekmtrWk, fleet }) {
    // узлы машин: база (БДО) + техместа загруженных выгрузок (createMetaCollector → nodes)
    const nodes = Object.assign({}, (base.wkNodes && base.wkNodes.orders) || {});
    uploads.forEach(u => Object.assign(nodes, (u.meta && u.meta.nodes) || {}));
    ...
    uploads.forEach(u => u.details.forEach(d => {
      const k = `${d.s}_${d.y}`, r = scheduleRowsFromDetail(d, nodes);
      fresh.set(k, r.rows);
