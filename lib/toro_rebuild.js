...
const ToroRebuild = (() => {
  ...
  function statusesFromControl(control) {
    ...
    decodeTable(control).forEach(r => {
      ...
      out.set(r.y + "|" + r.order, {
        phase: STAGE_PHASE[r.stage], copy: !!r.copy, kind: r.kind || "", kindText: r.kindText || "",
  ...
  function buildControl(scheduleRows, status, fleet, asOf, baseMeta) {
  ...
  function rebuild({ base, uploads = [], stockJson, ekmtrWk, fleet }) {
    ...
    uploads.forEach(u => {
      ...
      m.orders.forEach((v, o) => {
        ordersOfYear.add(o);
        status.set(y + "|" + o, { phase: phaseOf(v.sys), copy: m.originals.has(o), kind: v.kind, kindText: v.kindText,
    ...
    const control = buildControl(scheduleRows, status, fleet, provision.meta.asOf, base.control.meta);
