...
def main():
    ...
    issues.insert(2, {
        ...
        "impact": (
            "Запасы, KPI и склады показываются по площадке. Обеспеченность сначала берёт склад своей площадки "
            "(под её работы в пределах срока поставки он защищён), запас другой площадки — только перемещением, "
            "которое выводится отдельно. Закупка общая: документ закупки не привязан к площадке потребности."
        ),
    })
