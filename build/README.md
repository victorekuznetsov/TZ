...
## Обновление без Python (вкладка «Обновление данных»)

Новые выгрузки SAP BW пересобираются прямо в браузере, Python не нужен:

1. Откройте отчёт → «Обновление данных».
2. Остатки: три файла (остатки MM-M03, ограниченный запас, закупка ALL) →
   «Пересобрать остатки». ТОРО: `M06_{площадка}_{год}.xlsx` — достаточно тех,
   что изменились (загруженные площадко-годы заменяют свои, остальные берутся
   из текущих данных) → «Пересобрать витрины». УСО: `МТР УСО {год} all.xlsx`.
3. Проверьте таблицу «Сейчас / После пересборки» и сверки аналитики →
   «Применить и сохранить в браузере». Обновление сохраняется в хранилище
   браузера (IndexedDB) и переживает перезагрузку; вверху отчёта — плашка.
4. Локально (отчёт открыт с диска, двойной щелчок по index.html) —
   «Записать в папку отчёта на диске» (Chrome / Edge): указать папку, где
   лежит index.html, файлы `data/` перезапишутся на месте; папка запоминается.
   Иначе — «Скачать архив data/ для публикации» и распаковать поверх папки
   отчёта (или залить `data/` в репозиторий), чтобы обновление увидели все.
   «Вернуть опубликованные данные» — сброс к файлам.

Пересобираются: `stock.json`, график план-факт (`schedule_*.local.js`),
статусы SAP и стадии, копии «Развития», признак ППМ, `control.json`,
`provision.json`, `repairs.json`, `order_text.json`, `uso_wk.json` —
`lib/stock_pipeline.js`, `lib/pm06_pipeline.js`, `lib/toro_rebuild.js`,
`lib/uso_pipeline.js`. Правила те же, что у скриптов ниже:
`tests/test_toro_rebuild.mjs` сверяет браузерную сборку с Python — из текущих
витрин, на выборке настоящей выгрузки (`tests/fixtures/pm06_sample.xlsx`) и,
с `RAW_PM06_DIR=<папка с M06_*.xlsx>`, на полных выгрузках (совпадение
полное). Для неизменных площадко-лет браузер берёт базу из витрин:
строки графика (с колонкой `method`), статусы из `control.json`,
`provision.meta.ppmFlags` и `provision.meta.contour` — поэтому скрипты ниже
их тоже пишут.

Python остаётся эталоном и нужен для того, чего в браузере нет: каталог,
LinkOne, база знаний, КТГ (`fleet.json`), презентации.
...
### Поиск по номеру и «Эффективность» — проверка

```
python3 tests/verify_mtr.py     # независимый пересчёт → tests/fixtures/mtr_expected.json (после смены данных)
node tests/test_mtr.mjs         # сверка ядра lib/mtr_core.js
```
