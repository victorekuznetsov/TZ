...
s = content('МТР подрядчика (УСО)',
            ...
            f'({d["uso"]["meta"]["orders"]} заказов, закрыто в ТОРО {d["uso"]["meta"]["closedOrders"]}). 2026 — на 14.09.2026.')
lw = 7.6
box(s, X0, Y0 + 0.05, lw, 5.2, fill=WHITE, radius=0.18)
...
if AN:
    ...
    s = content('Контроль планирования: план 2027 и оперативный план',
                'Только заказы групп планирования ТОРО 100 Механика и 200 Энергетика — их планирует «Развитие» '
                f'({pct(sc.get("devShareNext"), 1)} плана {AN["next"]}); группы 300–900 — службы БЕ (заказчика). '
                'Цепочка ПЛАН → СГПЛ → ССПЛ → СГГС → деблокирование; статусы — доля плана в рублях.')
    ...
    tiles = [(pct(P['approvedShare']), f'плана {nxt} согласовано', f'{mln(P["approvedPlan"])} из {mln(P["nextPlan"])} млн ₽'),
             ...
             (sp(P['notReleasedStarted']['n']), 'начало прошло, не деблок.', f'{mln(P["notReleasedStarted"]["plan"])} млн ₽ 2026'),
    ...
    be_next = sum(g['next'] for g in sc.get('groups', []) if not g['dev'])
    tb(s, X0, 6.5, W, 0.3, [f'Только заказы групп планирования ТОРО 100 Механика и 200 Энергетика — {pct(sc.get("devShareNext"), 1)} плана {nxt}; '
                            f'группы 300–900 — службы БЕ (заказчика), {mln(be_next, 1)} млн ₽ плана {nxt}, в оценку планирования не входят.'],
       size=10, color=MUTED)
...
lims = [
    ...
    ('Стоимость закупок — в рублях', '«Общая стоимость» выгрузки закупки — в рублях: у позиций прайса ДП это цена в юанях × курс '
                                     'пересчёта ≈ 12,48 ₽/¥; валюта документа (ZCNY, ZUSD) — справочно.'),
]
cw2, ch2 = (W - 0.6) / 3, 2.5
